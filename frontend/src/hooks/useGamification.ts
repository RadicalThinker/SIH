...
export const useGamification = () => {
      ...
      if (response.ok) {
        ...
        if (result.newAchievements && result.newAchievements.length > 0) {
          ...
          setGamificationState(prev => ({
            ...
            recentAchievements: [...result.newBadges, ...prev.recentAchievements].slice(0, 3)
          }))
        }
      }
    } catch (error) {
  ...
  const showAchievementNotification = (achievement: Achievement) => {
    toast.success(
      <div className="flex items-center space-x-3">
        ...
        <div>
          <div className="font-semibold">{t('achievements.newBadge')}</div>
  ...
  const getAchievementProgress = (achievement: Achievement): number => {
    ...
    switch (criteria.type) {
      ...
      case 'completion':
        return Math.min(100, (stats.lessonsCompleted / criteria.threshold) * 100)
  ...
  const isAchievementEarned = (achievementId: string): boolean => {
    return gamificationState.earnedBadges.some(badge => badge.achievementId === achievementId)
  }

  const getLeaderboard = async (scope: 'class' | 'school' | 'global' = 'class') => {
    try {
      const response = await fetch(`/api/gamification/leaderboard?scope=${scope}`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      })
